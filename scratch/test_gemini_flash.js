...
async function testGeminiFlash() {
  console.log("Testing gemini-2.0-flash speed...");
  const start = Date.now();
  const googleUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${defaultGeminiKey}`;
  ...
  try {
    ...
    console.log(`Gemini Flash status ${res.status} in ${duration}s`);
