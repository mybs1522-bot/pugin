...
export class ProcessRecorder {
    ...
    try {
      ...
      this.mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          console.error("[ProcessRecorder] Save error:", err);
        }
      };
  ...
  private drawFrame(t: number) {
    ...
    if (t < 1.8) {
      this.drawSplashPhase(ctx, w, h, t / 1.8);
    } else if (t < 4.5) {
      this.drawViewportPhase(ctx, w, h, (t - 1.8) / 2.7);
    } else if (t < 9.0) {
      this.drawRenderPhase(ctx, w, h, (t - 4.5) / 4.5);
    } else if (t < 12.0) {
      this.drawSliderPhase(ctx, w, h, (t - 9.0) / 3.0);
    } else {
      this.drawWalkthroughPhase(ctx, w, h, (t - 12.0) / 2.5);
    }
  ...
  private drawSliderPhase(
    ...
    if (!this.imgVp || !this.imgRnd) return;
    ...
    this.drawFloatingTag(ctx, marginX, 24, "Raw SketchUp", "#3b82f6");
    this.drawFloatingTag(ctx, w - 240, 24, "4K Photoreal Render", "#10b981");
  }

  private drawWalkthroughPhase(
    ctx: CanvasRenderingContext2D,
    w: number,
    h: number,
    p: number
    ...
    if (this.videoEl && this.videoEl.readyState >= 2) {
      try {
        ctx.drawImage(this.videoEl, marginX, marginY, viewW, viewH);
      } catch (_) {
        if (this.imgRnd)
          this.drawImageContain(
            ctx,
    ...
    return new Promise((resolve) => {
      ...
      if (this.mediaRecorder && this.mediaRecorder.state !== "inactive") {
        this.mediaRecorder.onstop = async () => {
          ...
          try {
            await setAsset("custom_process_recording", finalBlob);
            if (this.options.onFinish) this.options.onFinish(finalBlob);
          } catch (_) {}
          resolve(finalBlob);
        };
